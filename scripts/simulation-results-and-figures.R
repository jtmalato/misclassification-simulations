...
gg_p80 +
  ...
  theme(legend.position = "none",
        ...
        axis.text.y = element_text(size = 13),
        panel.grid.major.y = element_line(colour = "gray", linetype = "dotted", size = 0.4),)
ggsave(here("figures/theta0-inference.pdf"),
       scale = 0.75, dpi = 320, width = 14, height = 13, device = cairo_pdf)
...
dt_st2 %>%
  ...
  scale_x_continuous(limits = c(0, 1),
                     breaks = seq(0,1,0.2),
                     labels = c("0","0.2","0.4","0.6","0.8", "1"),
                     expand = expansion(add = c(0.08, 0))) +
  labs(x = expression(paste("Misclassification rate,")~gamma),
       y = "Probability of rejecting null hypothesis") +
  geom_line(aes(group = snp, col = signif), lwd = 1.1) +
  geom_text(data = dt_st2[misrate == 0],
            aes(label = snp),
            vjust = c(0.2, 0.2, 1.1, 0.3, 0), hjust = 1.05, size = 4) +
  scale_colour_manual(values = col_line) +
  geom_hline(yintercept = c(0.05, 0.8), lty = 2) +
  guides(colour = guide_legend(nrow = 1, override.aes = list(size=2, alpha = 0.9))) +
  theme(legend.position = "none",
...
ggcl +
  theme_png() +
  theme(legend.position = "none")
ggsave(here("figures/simulations-cliff2019.png"),
       dpi = 320, width = 10, height = 10, scale = 1)
ggcl +
  theme_png_no_background() +
  theme(legend.position = "none")
ggsave(here("figures/simulations-cliff2019-no-background.png"),
       dpi = 320, width = 10, height = 10, scale = 1)
ggcl +
  theme_pdf() +
  theme(legend.position = "none")
ggsave(here("figures/simulations-cliff2019.pdf"),
       dpi = 320, width = 10, height = 10, scale = 1, device = cairo_pdf)
